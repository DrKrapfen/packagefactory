<#
.SYNOPSIS
PSAppDeployToolkit - Installs or uninstalls the network printer \\tsaps1\tsaPR-P3.

.DESCRIPTION
Connects (or removes) the shared network printer \\tsaps1\tsaPR-P3 and writes a
detection marker (TSAPrinterTSAPR-P3.ps1.tag). Migrated from PSADT v3 to v4.
...
$adtSession = @{
    # App variables.
    AppVendor = 'TSA'
    AppName = 'TSA Drucker Installieren TSAPR-P3'
    AppVersion = '1.0.2'
    AppArch = 'x64'
    AppLang = 'DE'
    AppRevision = '01'
...
function Install-ADTDeployment
{
    ...
    try {
        Write-ADTLogEntry -Message "Starting installation of network printer \\tsaps1\tsaPR-P3" -Source $deployAppScriptFriendlyName
        ...
        if ($existingPrinter) {
            Write-ADTLogEntry -Message "Printer \\tsaps1\tsaPR-P3 already exists. Removing existing printer..." -Source $deployAppScriptFriendlyName
            Remove-Printer -Name "\\tsaps1\tsaPR-P3" -ErrorAction SilentlyContinue
        }

        # Add the network printer
        Write-ADTLogEntry -Message "Adding network printer \\tsaps1\tsaPR-P3" -Source $deployAppScriptFriendlyName
        ...
        if ($newPrinter) {
            Write-ADTLogEntry -Message "Successfully installed printer \\tsaps1\tsaPR-P3" -Source $deployAppScriptFriendlyName

            # Set as default printer (optional - uncomment if needed)
            # Set-Printer -Name "\\tsaps1\tsaPR-P3" -Default
            # Write-ADTLogEntry -Message "Set \\tsaps1\tsaPR-P3 as default printer" -Source $deployAppScriptFriendlyName
        ...
        Write-ADTLogEntry -Message "Failed to install printer \\tsaps1\tsaPR-P3. Error: $($_.Exception.Message)" -Severity 3 -Source $deployAppScriptFriendlyName
        throw $_
    }
    finally {
        Stop-Transcript
...
function Uninstall-ADTDeployment
{
    ...
    try {
        Write-ADTLogEntry -Message "Starting uninstallation of printer \\tsaps1\tsaPR-P3" -Source $deployAppScriptFriendlyName

        # Check if printer exists
        $existingPrinter = Get-Printer -Name "\\tsaps1\tsaPR-P3" -ErrorAction SilentlyContinue

        if ($existingPrinter) {
            Write-ADTLogEntry -Message "Removing printer \\tsaps1\tsaPR-P3" -Source $deployAppScriptFriendlyName
            Remove-Printer -Name "\\tsaps1\tsaPR-P3" -ErrorAction Stop
            Write-ADTLogEntry -Message "Successfully removed printer \\tsaps1\tsaPR-P3" -Source $deployAppScriptFriendlyName
        } else {
            Write-ADTLogEntry -Message "Printer \\tsaps1\tsaPR-P3 not found, nothing to remove" -Source $deployAppScriptFriendlyName
        }
        ...
        Write-ADTLogEntry -Message "Failed to uninstall printer tsaPR-P3. Error: $($_.Exception.Message)" -Severity 3 -Source $deployAppScriptFriendlyName
        throw $_
    }
    finally {
        Stop-Transcript
